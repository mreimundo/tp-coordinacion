...
| Cola dedicada por Sum para broadcast de EOF (`{SUM_PREFIX}_{i}_eof`) | Fanout exchange con una queue exclusiva por Sum | A priori ambos enfoques son válidos. El exchange es más natural para fanout dinámico, pero igualmente requiere lógica adicional, ya que la instancia que recibe el EOF original y publica en el exchange también recibe su propia copia, por lo que necesita distinguir si el EOF que llegó a su queue exclusiva fue generado por ella misma y no volver a broadcastearlo (algún handling del escenario). Las colas dedicadas evitan este caso: el broadcast es siempre de uno a todos, y cada Sum consume únicamente su propia cola sin ambigüedad. Esta ventaja se sostiene porque `SUM_AMOUNT` es estático y conocido en el arranque — si el número de instancias pudiera variar en ejecución, el exchange sería más adecuado |
| Multiplexado de colas en un único channel (`add_queue_consumer` sobre la clase concreta) | Thread separado consumiendo la cola de EOF, o dos instancias de middleware con `start_consuming` bloqueante cada una | Un thread adicional requeriría locks sobre el estado del nodo y violaría la garantía de thread-safety de pika (dos threads llamando a operaciones del channel simultáneamente). Dos instancias bloqueantes necesitarían threads de todas formas. El multiplexado en el mismo channel es secuencial por diseño de pika: los callbacks de ambas colas nunca se ejecutan en paralelo. `add_queue_consumer` se agregó solo a la clase concreta (`MessageMiddlewareQueueRabbitMQ`), sin modificar la interfaz abstracta (`MessageMiddlewareQueue`), es decir el punto de entrada de consumo sigue siendo único |
